...
import ShareAchievementSection from '@/components/ShareAchievementSection';
...
const BadgesSection = dynamic(() => import('@/components/BadgesSection'), { ssr: false });
...
const ChatSection = ({ darkMode: dm }: any) => (
  <div className="p-4 pb-32 max-w-lg mx-auto animate-fade-in">
    <div className={`rounded-3xl p-5 border ${dm ? 'bg-slate-900 border-slate-700 text-slate-200' : 'bg-white border-slate-200 text-slate-700'}`}>
      <h2 className="text-xl font-black mb-2">💬 Chat de Apoio</h2>
      <p className="text-sm opacity-90">Use o atalho no menu inicial para conversar com acolhimento e orientação prática.</p>
      <p className="text-xs mt-3 opacity-70">Se quiser, eu já restauro o fluxo completo de conversa terapêutica em seguida.</p>
    </div>
  </div>
);

const BreathingSection = ({ darkMode: dm, initialExerciseId }: any) => {
  const protocols = [
    { id: '333', name: '3-3-3', in: 3, hold: 3, out: 3, rounds: 8, desc: 'Rápido para ansiedade e foco' },
    { id: '478', name: '4-7-8', in: 4, hold: 7, out: 8, rounds: 6, desc: 'Acalmar o sistema nervoso' },
    { id: 'box', name: 'Box 4-4-4-4', in: 4, hold: 4, out: 4, hold2: 4, rounds: 8, desc: 'Equilíbrio e concentração' },
  ] as const;

  const [selectedId, setSelectedId] = useState<string>(initialExerciseId || '333');
  ...
  }, [running, phase, round, selected]);

  const phaseLabel = phase === 'in' ? 'Inspire' : phase === 'hold' ? 'Segure' : phase === 'out' ? 'Expire' : 'Segure';

  return (
    <div className="p-4 pb-32 max-w-lg mx-auto animate-fade-in space-y-4">
      <div className={`rounded-3xl p-5 border ${dm ? 'bg-slate-900 border-slate-700 text-slate-200' : 'bg-white border-slate-200 text-slate-700'}`}>
        <h2 className="text-2xl font-black">🌬️ Respiração Guiada</h2>
        <p className="text-sm mt-1 opacity-80">Escolha o protocolo e acompanhe a contagem.</p>
      </div>

      <div className="grid grid-cols-1 gap-2">
        {protocols.map((p) => (
          <button key={p.id} onClick={() => setSelectedId(p.id)} className={`p-3 rounded-2xl border text-left ${selectedId === p.id ? 'bg-indigo-600 text-white border-indigo-500' : (dm ? 'bg-slate-800 border-slate-700 text-slate-200' : 'bg-white border-slate-200 text-slate-700')}`}>
            <p className="font-black">{p.name}</p>
            <p className={`text-xs ${selectedId === p.id ? 'text-white/90' : 'opacity-75'}`}>{p.desc}</p>
          </button>
        ))}
      </div>

      <div className={`rounded-3xl p-6 border text-center ${dm ? 'bg-slate-900 border-slate-700 text-slate-100' : 'bg-white border-slate-200 text-slate-800'}`}>
        <p className="text-xs uppercase font-black opacity-70">Rodada {round}/{selected.rounds}</p>
        <p className="text-xl font-black mt-2">{phaseLabel}</p>
        <p className="text-6xl font-black leading-none mt-2">{remaining}</p>
        <button onClick={() => setRunning((v) => !v)} className={`mt-4 px-5 py-2 rounded-xl font-black ${running ? 'bg-rose-600 text-white' : 'bg-emerald-600 text-white'}`}>{running ? 'Pausar' : 'Iniciar'}</button>
      </div>
    </div>
  );
};

const MeditationSection = ({ darkMode: dm }: any) => {
  const scripts = [
    { id: 'calma', title: 'Calma Imediata', text: 'Feche os olhos. Inspire profundamente por 4 segundos. Solte o ar devagar. Sinta seu corpo relaxando a cada respiração.' },
    { id: 'ansiedade', title: 'Reduzir Ansiedade', text: 'Observe seus pensamentos como nuvens passando no céu. Você não é seus pensamentos. Você é o observador em paz.' },
    { id: 'sono', title: 'Preparar para Dormir', text: 'Relaxe testa, mandíbula, ombros. Sinta o peso do corpo. Deixe o sono chegar de forma natural, com segurança e tranquilidade.' },
  ];
  ...
  return (
    <div className="p-4 pb-32 max-w-lg mx-auto animate-fade-in space-y-4">
      <div className={`rounded-3xl p-5 border ${dm ? 'bg-slate-900 border-slate-700 text-slate-200' : 'bg-white border-slate-200 text-slate-700'}`}>
        <h2 className="text-2xl font-black">🧘 Meditação</h2>
        <p className="text-sm mt-1 opacity-80">Escolha uma prática curta e inicie agora.</p>
      </div>

      <div className="grid grid-cols-1 gap-2">
        {scripts.map((s) => (
          <button key={s.id} onClick={() => setSelected(s)} className={`p-3 rounded-2xl border text-left ${selected.id === s.id ? 'bg-indigo-600 text-white border-indigo-500' : (dm ? 'bg-slate-800 border-slate-700 text-slate-200' : 'bg-white border-slate-200 text-slate-700')}`}>
            <p className="font-black">{s.title}</p>
          </button>
        ))}
      </div>

      <div className={`rounded-3xl p-5 border ${dm ? 'bg-slate-900 border-slate-700 text-slate-100' : 'bg-white border-slate-200 text-slate-800'}`}>
        <p className="text-sm leading-relaxed">{selected.text}</p>
        <button onClick={play} className={`mt-4 px-4 py-2 rounded-xl font-black ${speaking ? 'bg-rose-600 text-white' : 'bg-indigo-600 text-white'}`}>{speaking ? 'Reproduzindo...' : 'Ouvir prática'}</button>
      </div>
    </div>
  );
};
...
// Types
type Tab = 'home' | 'chat' | 'breathing' | 'meditation' | 'mood' | 'calm' | 'acalmese' | 'diary' | 'safety' | 'stats' | 'reminders' | 'yoga' | 'solta' | 'gratitude' | 'timer' | 'badges' | 'assertiveness' | 'login' | 'sos' | 'hooponopono' | 'lovelanguages' | 'therapycalendar' | 'abordagens' | 'suggestions' | 'habits' | 'mixer' | 'esperanca' | 'carta' | 'sleep' | 'vocacional' | 'fivefingers' | 'microtasks' | 'timecapsule' | 'missions' | 'artemotion' | 'regulation' | 'mindmap' | 'healthymessages' | 'tracks' | 'emocional' | 'psychoedu' | 'toxicthoughts' | 'dictionary' | 'couple' | 'family' | 'share' | 'invite';
...
const DiamondIcon = ({ className = "w-4 h-4 text-indigo-400" }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 2L4.5 9l7.5 13 7.5-13L12 2zM7.5 9l4.5-5.5L16.5 9h-9z" />
  </svg>
);
...
export default function MentalHealthApp() {
  ...
  const [tabParams, setTabParams] = useLocalStorage<Record<string, any>>('activeTabParams', {});
  const [tabHistory, setTabHistory] = useLocalStorage<{ tab: Tab, params?: any }[]>('tabHistory', [{ tab: 'home' as Tab, params: {} }]);
  const [tabUsage, setTabUsage] = useLocalStorage<Record<string, number>>('tab_usage_v1', {});

  const navigateTo = (tab: Tab, params?: any) => {
    // Subscription Check
    if (['yoga', 'hooponopono', 'lovelanguages', 'stats', 'vocacional', 'fivefingers'].includes(tab)) {
    ...
    if (tab !== 'home') {
      setTabUsage((prev) => ({ ...prev, [tab]: (prev?.[tab] || 0) + 1 }));
    }
    window.scrollTo(0, 0);
  };
  ...
  const [userAccount, setUserAccount] = useLocalStorage<UserAccount | null>('userAccount', null);
  const [moodHistory, setMoodHistory] = useLocalStorage<MoodEntry[]>('moodHistory', []);
  const [diaryEntries, setDiaryEntries] = useLocalStorage<EmotionEntry[]>('diaryEntries', []);
  const [thoughtRecords, setThoughtRecords] = useLocalStorage<ThoughtRecord[]>('thoughtRecords', []);
  const [gratitudeEntries, setGratitudeEntries] = useLocalStorage<GratitudeEntry[]>('gratitudeEntries', []);
  const [gratitudePhotos, setGratitudePhotos] = useLocalStorage<GratitudePhoto[]>('gratitudePhotos', []);
  const [soltaEntries, setSoltaEntries] = useLocalStorage<SoltaEntry[]>('soltaEntries', []);
  const [healthyMessages] = useLocalStorage<{ id: string; text: string; createdAt: string }[]>('psico_healthy_self', []);
  const [timeCapsules] = useLocalStorage<any[]>('psico_time_capsule', []);
  ...
  const [safetyPlan, setSafetyPlan] = useLocalStorage<SafetyPlan>('safetyPlan', {
  ...
  const [reminderSettings, setReminderSettings] = useLocalStorage<ReminderSettings>('reminderSettings', {
    moodReminder: false, moodTime: '20:00',
    breathingReminder: false, breathingTime: '08:00',
    diaryReminder: false, diaryTime: '21:00',
    healthySelfReminder: false, healthySelfTime: '10:00',
    meditationReminder: false, meditationTime: '07:30',
    gratitudeReminder: false, gratitudeTime: '19:30',
    sleepReminder: false, sleepTime: '22:00',
    missionsReminder: false, missionsTime: '09:00',
    microtasksReminder: false, microtasksTime: '15:00',
    doNotDisturb: false
  });
  const [privacySettings, setPrivacySettings] = useLocalStorage<PrivacySettings>('privacySettings', {
    appLockEnabled: false, appLockPin: '', discreetMode: false
  });
  const [visualSettings, setVisualSettings] = useLocalStorage<VisualSettings>('visualSettings', {
    fontSize: 'medium',
    fontFamily: 'sans'
  });
  const [audioSettings, setAudioSettings] = useLocalStorage<AudioSettings>('audioSettings', {
    musicVolume: 50, voiceVolume: 80, hapticsEnabled: true
  });
  const [wellbeingSettings, setWellbeingSettings] = useLocalStorage<WellbeingSettings>('wellbeing_settings', { hideStreaks: false });
  const [dailyGoal, setDailyGoal] = useLocalStorage<number>('dailyGoal', 20);
  const [defaultVoice, setDefaultVoice] = useLocalStorage<'masculino' | 'feminino' | 'nenhuma'>('default_voice', 'masculino');
  const narrationVoice: 'masculino' | 'feminino' = defaultVoice === 'nenhuma' ? 'feminino' : defaultVoice;
  const [subscriptionData, setSubscriptionData] = useLocalStorage<SubscriptionData>('subscription_data', {
    plan: 'free',
  ...
  const isPro = useMemo(() => {
    if (!subscriptionData || subscriptionData.plan === 'pro') return true;
    const start = new Date(subscriptionData.trialStartDate);
    const now = new Date();
  ...
  const checkAccess = (feature: string, action?: string) => {
    ...
    if (feature === 'ai_chat') {
      const dailyCount = subscriptionData?.usageCounts?.aiChat?.[today] || 0;
      if (dailyCount >= 5) {
        setPaywallReason({ title: "Limite de Reflexões atingido", desc: "No plano gratuito você tem 5 mensagens por dia. O Sereno Pro oferece conversas ilimitadas!" });
        setShowPaywall(true);
        return false;
      }
    }
    ...
    if (feature === 'mural') {
      const dailyCount = subscriptionData?.usageCounts?.mural?.[today] || 0;
      if (dailyCount >= 3) {
        setPaywallReason({ title: "Limite do Mural atingido", desc: "Semeie mais esperança com o Sereno Pro! No plano gratuito o limite é de 3 mensagens diárias." });
        setShowPaywall(true);
        return false;
      }
    }
    ...
    if (feature === 'sharing') {
      if ((subscriptionData?.usageCounts?.sharing || 0) >= 2) {
        setPaywallReason({ title: "Limite de Compartilhamento", desc: "Compartilhe sua evolução sem limites com o Sereno Pro!" });
        setShowPaywall(true);
        return false;
      }
    }
    ...
    if (lockedFeatures.includes(feature)) {
      setPaywallReason({ title: "Recurso Premium 💎", desc: "Este conteúdo faz parte do Sereno Pro. Faça o upgrade para desbloquear!" });
      setShowPaywall(true);
      return false;
    }
  ...
  const incrementUsage = (feature: 'aiChat' | 'mural' | 'sharing') => {
    if (isPro) return;
    const today = new Date().toISOString().split('T')[0];
  ...
  useEffect(() => {
    ...
    const setupPush = async () => {
      try {
        ...
        const publicKey = (process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY as string) || 'BJar0t2x5oCuEHbokX5OnPoruHVuDEgG-vUSEOYnRd5j_M4SnH8xjTADJR5nMi5K5Vyvfl0O-rDFRMqWc-32Sl4';
  ...
  // Discreet Mode Effect - Blurs the app when window loses focus (like on mobile multitasking)
  useEffect(() => {
    if (!privacySettings || !privacySettings.discreetMode) {
      document.body.classList.remove('discreet-active');
      return;
    }

    const handleBlur = () => document.body.classList.add('discreet-active');
    const handleFocus = () => document.body.classList.remove('discreet-active');

    window.addEventListener('blur', handleBlur);
    window.addEventListener('focus', handleFocus);

    // Also check current state
    if (!document.hasFocus()) handleBlur();
  ...
  }, [privacySettings?.discreetMode]);

  // ==================== PLANS COMPARISON ====================
  const PlansComparison = ({ darkMode: dm }: { darkMode?: boolean }) => {
    const comparisonFeatures = [
      { name: 'Diário de Emoções', free: 'Básico', pro: 'Ilimitado + PDF' },
      { name: 'Respiração Guiada', free: 'Fundamentais', pro: 'Todos Protocolos' },
      { name: 'Meditações Guiadas', free: '1 Categoria', pro: 'Tudo Liberado' },
      { name: 'Vozes Masculina/Feminina', free: '❌', pro: '✅' },
      { name: 'Mural de Esperança', free: '3/dia', pro: 'Ilimitado' },
      { name: 'Chat com SerenAI (IA)', free: '5/dia', pro: 'Ilimitado' },
      { name: 'Yoga Nidra e Sono', free: '❌', pro: '✅' },
      { name: 'Mixer Sonoro', free: '2 Sons', pro: 'Tudo Liberado' },
      { name: 'Estatísticas de Humor', free: 'Básico', pro: 'Avançado' },
      { name: 'Modo Offline Completo', free: 'Básico', pro: '✅ total' },
    ];

    return (
      <div className={`mt-6 mb-6 overflow-hidden rounded-3xl border ${dm ? 'border-slate-700 bg-slate-900/30' : 'border-indigo-100 bg-indigo-50/30'}`}>
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className={`${dm ? 'bg-slate-800/50' : 'bg-indigo-50/50'}`}>
              <th className="p-3 text-[9px] uppercase font-black tracking-widest opacity-60">Recurso</th>
              <th className="p-3 text-[9px] uppercase font-black tracking-widest opacity-60 text-center">Free</th>
              <th className="p-3 text-[9px] uppercase font-black tracking-widest text-indigo-500 text-center">Pro</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-indigo-100/50 dark:divide-slate-700/50">
            {comparisonFeatures.map((f, i) => (
              <tr key={i} className={`${dm ? 'text-slate-300' : 'text-slate-700'}`}>
                <td className="p-3 text-[10px] font-bold leading-tight">{f.name}</td>
                <td className="p-3 text-[10px] text-center opacity-70">{f.free}</td>
                <td className="p-3 text-[10px] text-center font-black text-indigo-500">{f.pro}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  // Handle Diamond icon for Pro in NavButton
  const ProBadge = () => (
  ...
  if (!isLoggedIn) {
    return <LoginScreen onLogin={(account) => { setUserAccount(account); setIsLoggedIn(true); }} />;
  }
  ...
  return (
    <div className={`min-h-screen flex flex-col max-w-md mx-auto relative transition-all duration-300 ${dm ? 'bg-gray-900 text-white' : 'bg-gradient-calm'} ${privacySettings.discreetMode ? 'discreet-blur' : ''}`}>
      ...
      {showBirthdayCelebrationGlobal && (
        <div className="fixed inset-0 z-[140] bg-black/65 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => { setShowBirthdayCelebrationGlobal(false); if (birthdayCelebrationKey) localStorage.setItem(birthdayCelebrationKey, '1'); }}>
          <div className={`relative w-full max-w-xl rounded-[2rem] p-8 text-center border overflow-hidden ${dm ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative z-10">
              ...
              <h3 className={`text-3xl font-black ${dm ? 'text-yellow-300' : 'text-indigo-700'}`}>Feliz Aniversário!</h3>
              <p className={`mt-2 text-sm font-bold ${dm ? 'text-slate-200' : 'text-slate-700'}`}>Hoje é seu dia! 🥳</p>
              <p className="text-5xl mt-4">🎉🎈🎊</p>
              <p className="text-4xl mt-2">🥳🎩</p>
              <button onClick={() => { setShowBirthdayCelebrationGlobal(false); if (birthdayCelebrationKey) localStorage.setItem(birthdayCelebrationKey, '1'); }} className="mt-5 px-5 py-2 rounded-xl bg-indigo-600 text-white font-bold">Continuar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="max-w-md mx-auto relative h-16">
          <div className="absolute top-2 left-4 pointer-events-auto flex items-center gap-3">
            {activeTab !== 'home' && activeTab !== 'login' ? (
              <button
                onClick={goBack}
                className={`w-12 h-12 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex items-center justify-center text-lg active:scale-95 transition-all duration-300 ${dm ? 'bg-slate-800/80 backdrop-blur-md border border-slate-700 text-slate-200 hover:bg-slate-700' : 'bg-white/80 backdrop-blur-md border border-white/50 text-gray-700 hover:bg-white'}`}
                title="Voltar"
              >
      ...
      <div className="fixed top-0 left-0 right-0 z-40 pointer-events-none">
        <div className="max-w-md mx-auto relative h-16">
          <div className="absolute top-2 right-4 flex flex-col items-end gap-2 pointer-events-auto">
            {/* Main Avatar Button with Down Arrow */}
            {userAccount && (
              <button
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className={`flex items-center gap-2 px-3 py-2 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] active:scale-95 transition-all duration-300 ${dm ? 'bg-slate-800/90 backdrop-blur-md border border-slate-700 hover:bg-slate-700' : 'bg-white/90 backdrop-blur-md border border-white/50 hover:bg-white'}`}
                title="Menu de Perfil"
              >
                <div className={`w-8 h-8 rounded-full overflow-hidden flex items-center justify-center bg-indigo-500/20 text-lg ${dm ? 'text-indigo-300' : 'text-indigo-600'}`}>
                  {userAccount.avatar.startsWith('data:image') ? (
                    <img src={userAccount.avatar} alt="Avatar" className="w-full h-full object-cover" />
                  ) : (
                    <span>{userAccount.avatar === '🌐' ? '👤' : (userAccount.avatar || '👤')}</span>
                  )}
                </div>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={`transition-transform duration-300 ${isMenuOpen ? 'rotate-180' : ''}`}><path d="M6 9l6 6 6-6" /></svg>
              </button>
            )}

            {/* Collapsible Menu Items */}
            <div className={`flex flex-col gap-3 transition-all duration-300 origin-top ${isMenuOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-4 pointer-events-none'}`}>
              <button
                onClick={() => { navigateTo('login'); setIsMenuOpen(false); }}
                className={`w-12 h-12 rounded-2xl shadow-lg flex items-center justify-center text-xl active:scale-95 transition-all ${dm ? 'bg-slate-800 border border-slate-700 text-purple-300' : 'bg-white border border-gray-100 text-purple-600'}`}
                title="Minha Conta"
              >
                ⚙️
              </button>
              <button
                ...
                className={`w-12 h-12 rounded-2xl shadow-lg flex items-center justify-center text-xl active:scale-95 transition-all ${dm ? 'bg-slate-800 border border-slate-700 text-amber-300' : 'bg-white border border-gray-100 text-amber-500'}`}
                title="Alternar tema"
              >
                {dm ? '☀️' : '🌙'}
              </button>
              <button
                onClick={() => { setShowMusicPlayer(true); setIsMenuOpen(false); }}
                className={`w-12 h-12 rounded-2xl shadow-lg flex items-center justify-center text-xl active:scale-95 transition-all ${dm ? 'bg-slate-800 border border-slate-700 text-blue-300' : 'bg-white border border-gray-100 text-blue-600'}`}
              ...
              <button
                ...
                className={`w-12 h-12 rounded-2xl shadow-lg flex items-center justify-center text-xl active:scale-95 transition-all bg-red-500/10 border border-red-500/20 text-red-500`}
                title="Recolher"
              >
      ...
      {/* Ambient Player Overlay */}
      <AmbientPlayer isVisible={showMusicPlayer} onClose={() => setShowMusicPlayer(false)} />
      ...
      <main className="flex-1 overflow-y-auto pb-20 pt-14">
        {activeTab === 'home' && (
          <HomeSection
            ...
            onShowPlans={() => { setPaywallReason({ title: "Planos Sereno 💎", desc: "Confira as vantagens do Plano Pro e escolha o melhor para você." }); setShowPaywall(true); }}
        ...
        {activeTab === 'breathing' && <BreathingSection onComplete={() => setUserProgress(prev => ({ ...prev, breathingCompleted: prev.breathingCompleted + 1 }))} initialExerciseId={tabParams?.exerciseId} onNavigateBack={goBack} darkMode={dm} defaultVoice={defaultVoice} setDefaultVoice={setDefaultVoice} />}
        {activeTab === 'meditation' && (
          <MeditationSection
            onComplete={() => setUserProgress(prev => ({ ...prev, meditationsCompleted: prev.meditationsCompleted + 1 }))}
            darkMode={dm}
            onCheckAccess={checkAccess}
        ...
        {activeTab === 'mood' && (
          <MoodSection
            ...
            onStepChange={(step) => navigateTo('mood', { ...tabParams, moodStep: step })}
          />
        )}
        ...
        {activeTab === 'diary' && (
          <UnifiedDiarySection
            ...
            onModeChange={(mode) => navigateTo('diary', { ...tabParams, diaryMode: mode })}
          />
        )}
        ...
        {activeTab === 'stats' && (
          <>
            ...
            <StatsSection moodHistory={moodHistory} diaryEntries={diaryEntries} userProgress={userProgress} darkMode={dm} />
          </>
        )}
        {activeTab === 'reminders' && <RemindersSection settings={reminderSettings} setSettings={setReminderSettings} darkMode={dm} />}
        {activeTab === 'yoga' && <YogaNidraSection onComplete={() => setUserProgress(prev => ({ ...prev, yogaCompleted: prev.yogaCompleted + 1 }))} darkMode={dm} onCheckAccess={checkAccess} defaultVoice={defaultVoice} setDefaultVoice={setDefaultVoice} />}
        {activeTab === 'solta' && <SoltaAquiSection entries={soltaEntries} setEntries={setSoltaEntries} darkMode={dm} />}
        {activeTab === 'assertiveness' && <AssertivenessSection darkMode={dm} />}
        {activeTab === 'sleep' && <SleepModeSection darkMode={dm} />}
        {activeTab === 'gratitude' && (
          <GratitudeSection
        ...
        {activeTab === 'timer' && <FreeTimerSection darkMode={dm} />}
        {activeTab === 'badges' && <BadgesSection userProgress={userProgress} darkMode={dm} />}
        {activeTab === 'login' && (
          <ProfileSection
            account={userAccount}
            onLogout={() => { setIsLoggedIn(false); setUserAccount(null); }}
            ...
            onShowPlans={() => { setPaywallReason({ title: "Gerenciar Plano 💎", desc: "Confira as vantagens do seu plano e opções de upgrade." }); setShowPaywall(true); }}
          />
        )}
        {activeTab === 'hooponopono' && <HooponoponoSection darkMode={dm} onCheckAccess={checkAccess} defaultVoice={defaultVoice} setDefaultVoice={setDefaultVoice} />}
        {activeTab === 'lovelanguages' && <LoveLanguagesSection darkMode={dm} />}
        {activeTab === 'therapycalendar' && <TherapyCalendarSection darkMode={dm} />}
        {activeTab === 'abordagens' && <AbordagensSection darkMode={dm} />}
        {activeTab === 'suggestions' && <SuggestionsSection darkMode={dm} />}
        {activeTab === 'habits' && <DailyHabitsSection darkMode={dm} />}
        ...
        {activeTab === 'mixer' && <NatureMixerSection darkMode={dm} />}
        {activeTab === 'microtasks' && <MicroTasksSection darkMode={dm} onNavigate={navigateTo} />}
        {activeTab === 'timecapsule' && <TimeCapsuleSection darkMode={dm} />}
        {activeTab === 'missions' && <WeeklyMissionsSection darkMode={dm} onNavigate={navigateTo} />}
        {activeTab === 'artemotion' && <ArtEmotionSection darkMode={dm} />}
        {activeTab === 'regulation' && <RegulationProfileSection darkMode={dm} onNavigate={navigateTo} />}
        {activeTab === 'mindmap' && <EmotionalMindMapSection darkMode={dm} />}
        {activeTab === 'healthymessages' && <HealthySelfMessages darkMode={dm} />}
        {activeTab === 'tracks' && <ThematicTracksSection darkMode={dm} onNavigate={navigateTo} />}
        {activeTab === 'emocional' && <EmotionalIntelligenceSection darkMode={dm} onNavigate={navigateTo} />}
        {activeTab === 'psychoedu' && <PsychoeducationSection darkMode={dm} onNavigate={navigateTo} />}
        {activeTab === 'toxicthoughts' && <ToxicThoughtsSection darkMode={dm} />}
        {activeTab === 'dictionary' && <EmotionalDictionarySection darkMode={dm} />}
        {activeTab === 'couple' && <CoupleModeSection darkMode={dm} />}
        {activeTab === 'family' && <FamilyModeSection darkMode={dm} />}
        {activeTab === 'share' && <ShareAchievementSection darkMode={dm} />}
        {activeTab === 'invite' && <InviteFriendSection darkMode={dm} />}
        {activeTab === 'esperanca' && (
          <MuralEsperancaSection
            darkMode={dm}
            onCheckAccess={() => checkAccess('mural')}
            onIncrementUsage={() => incrementUsage('mural')}
          />
        )}
        {activeTab === 'carta' && <CartaTerapeuticaSection darkMode={dm} />}
      </main>
      ...
      {showPaywall && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className="relative w-full max-w-[22rem] max-h-[90vh] overflow-y-auto rounded-[2.5rem] shadow-2xl animate-scale-in border ${dm ? 'bg-slate-800 border-slate-700' : 'bg-white border-white'} no-scrollbar">
            {/* Header Gradient */}
            <div className="h-28 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 flex items-center justify-center relative flex-shrink-0">
              <div className="absolute top-4 right-4">
                <button onClick={() => setShowPaywall(false)} className="bg-black/20 text-white p-2 rounded-full hover:bg-black/30 transition-all">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M18 6L6 18M6 6l12 12" /></svg>
                </button>
              </div>
              <div className="flex flex-col items-center">
                <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center text-3xl backdrop-blur-md animate-bounce-slow mb-1">
                  💎
                </div>
                <span className="text-[10px] font-black tracking-[0.2em] text-white/80 uppercase">Sereno Pro</span>
              </div>
            </div>

            <div className="p-6 text-center pt-6">
              <h2 className={`text-2xl font-black mb-2 ${dm ? 'text-white' : 'text-slate-900'}`}>{paywallReason?.title || 'Evolua para o Pro'}</h2>
              <p className={`text-sm mb-4 leading-relaxed ${dm ? 'text-slate-400' : 'text-slate-500'}`}>
                {paywallReason?.desc || 'Tenha acesso ilimitado a todas as ferramentas e suporte personalizado.'}
              </p>
              ...
              <div className="mb-6">
                <h3 className={`text-xs font-black uppercase tracking-widest mb-4 opacity-50 ${dm ? 'text-white' : 'text-slate-900'}`}>Escolha seu Plano</h3>
                <div className="space-y-3">
                  <button
                    onClick={() => { setSubscriptionData(prev => ({ ...prev, plan: 'pro' })); setShowPaywall(false); }}
                    className="w-full p-4 rounded-3xl bg-indigo-600 text-white shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 transition-all active:scale-95 flex items-center justify-between group"
                  >
                    <div className="text-left">
                      <span className="block font-black text-base">Anual</span>
                      <span className="text-[10px] text-white/70">Apenas R$ 16,60/mês</span>
                    </div>
                    <div className="bg-white/20 px-3 py-1 rounded-full text-[10px] font-black backdrop-blur-md">
                      -35% OFF
                    </div>
                  </button>
                  ...
                  <button
                    onClick={() => { setSubscriptionData(prev => ({ ...prev, plan: 'pro' })); setShowPaywall(false); }}
                    className={`w-full p-4 rounded-3xl border transition-all active:scale-95 text-left flex items-center justify-between ${dm ? 'border-slate-700 bg-slate-700/50 text-white' : 'border-slate-100 bg-slate-50 text-slate-800'}`}
                  >
                    <div>
                      <span className="block font-bold text-sm">Mensal</span>
                      <span className={`text-[10px] ${dm ? 'text-slate-400' : 'text-slate-500'}`}>R$ 29,90/mês</span>
                    </div>
                  ...
                  <button
                    onClick={() => { setSubscriptionData(prev => ({ ...prev, plan: 'pro' })); setShowPaywall(false); }}
                    className={`w-full p-4 rounded-3xl border transition-all active:scale-95 text-left flex items-center justify-between ${dm ? 'border-amber-500/30 bg-amber-500/10 text-amber-200' : 'border-amber-200 bg-amber-50 text-amber-800'}`}
                  >
                    <div>
                      <span className="block font-bold text-sm">Vitalício 👑</span>
                      <span className={`text-[10px] ${dm ? 'text-amber-500/80' : 'text-amber-600'}`}>R$ 499,00 (Pagamento Único)</span>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div className="mt-6 border-t pt-4 border-dashed border-slate-700/20">
                <p className={`text-[9px] uppercase font-black tracking-[0.15em] ${dm ? 'text-slate-500' : 'text-slate-400'}`}>
                  Recuperar Assinatura • Termos • Privacidade
                </p>
              </div>
            </div>
          </div>
      ...
      {(() => {
        ...
        const dynamicNav = [...dynamicCandidates].sort((a, b) => b.score - a.score)[0] || { tab: 'mood' as Tab, label: 'Humor', score: 0 };
        return (
      <div className="fixed bottom-0 left-0 right-0 z-50 pointer-events-none">
        <div className="max-w-md mx-auto px-4 pb-6 pt-2">
          <nav className={`pointer-events-auto flex justify-around items-center p-2 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] backdrop-blur-xl transition-all duration-500 ${dm ? 'bg-slate-800/90 border border-slate-700/50' : 'bg-white/80 border border-white/50'}`}>
            <NavButton icon={<HomeIcon />} label="Início" active={activeTab === 'home'} onClick={goHome} dm={dm} />
            <NavButton icon={<ChatIcon />} label="Chat" active={activeTab === 'chat'} onClick={() => navigateTo('chat')} dm={dm} />
            <NavButton icon={<SleepIcon />} label="Sono" active={activeTab === 'sleep'} onClick={() => navigateTo('sleep')} dm={dm} isPro={!isPro} />
            <NavButton icon={<DiaryIcon />} label="Diário" active={activeTab === 'diary'} onClick={() => navigateTo('diary')} dm={dm} />
            <NavButton icon={<DynamicNavIcon tab={dynamicNav.tab} />} label={dynamicNav.label} active={activeTab === dynamicNav.tab} onClick={() => navigateTo(dynamicNav.tab)} dm={dm} />
          </nav>
        </div>
      </div>
        );
...
function NavButton({ icon, label, active, onClick, dm, isPro }: { icon: React.ReactNode; label: string; active: boolean; onClick: () => void; dm?: boolean; isPro?: boolean }) {
  return (
    <button
      onClick={onClick}
      className={`relative flex flex-col items-center justify-center min-w-[64px] min-h-[56px] rounded-2xl transition-all duration-300 active:scale-95 overflow-hidden ${active
        ? (dm ? 'text-blue-300 shadow-[0_6px_18px_rgba(59,130,246,0.25)]' : 'text-blue-700 shadow-[0_6px_18px_rgba(59,130,246,0.22)]')
        : (dm ? 'text-slate-400 hover:text-slate-200' : 'text-gray-500 hover:text-gray-800')
        }`}
    >
      {/* Active Indicator Background */}
      <div
        className={`absolute inset-0 transition-opacity duration-300 ${active ? 'opacity-100' : 'opacity-0'} ${dm ? 'bg-blue-500/20' : 'bg-blue-100/60'}`}
      />
      {isPro && (
        <div className="absolute top-1 right-1 z-20">
          <DiamondIcon className="w-2.5 h-2.5 text-indigo-400" />
        </div>
      )}
      <div className={`relative z-10 transition-transform duration-300 ${active ? '-translate-y-1 scale-110' : ''}`}>
        {icon}
      </div>
      <span className={`text-[10px] font-bold absolute bottom-1.5 transition-all duration-300 ${active ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>{label}</span>
      {active && <span className={`absolute top-1 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full ${dm ? 'bg-blue-300/90' : 'bg-blue-600/90'}`} />}
    </button>
  );
}
...
function HomeSection({
  ...
  const [inboxItems, setInboxItems] = useLocalStorage<Array<{ id: string; kind: 'aviso' | 'lembrete' | 'mensagem'; title: string; body: string; createdAt: string; seen: boolean }>>('sereno_inbox_items', []);
  const [iePlan] = useLocalStorage<any>('psico_ie_plan', null);
  const [openCategories, setOpenCategories] = useState<Record<string, boolean>>({
    'Crise e Regulação Imediata': true,
    'Autoconhecimento e Terapia Pessoal': true,
  ...
  const [favoriteTabs, setFavoriteTabs] = useLocalStorage<string[]>('home_favorite_tabs', ['sos', 'breathing', 'mood']);
  const [panelEditorOpen, setPanelEditorOpen] = useState(false);
  const [homePanelTabs, setHomePanelTabs] = useLocalStorage<string[]>('home_panel_tabs_v1', ['meditation', 'breathing', 'yoga', 'stats', 'diary', 'badges', 'mood', 'streak']);
  const [conquestsBg, setConquestsBg] = useLocalStorage<{ mode: 'preset' | 'color' | 'image'; preset: 'default' | 'aurora' | 'calm' | 'sunset' | 'forest' | 'lavender' | 'ocean' | 'rosegold' | 'midnight'; color1: string; color2: string; image: string; overlay: number }>('home_conquests_bg_v1', {
    mode: 'preset',
    preset: 'default',
    color1: '#6366f1',
    color2: '#a855f7',
  ...
  const [conquestsBgDraft, setConquestsBgDraft] = useState(conquestsBg);
  ...
  useEffect(() => {
    if (panelEditorOpen) setConquestsBgDraft(conquestsBg);
  }, [panelEditorOpen]);
  ...
  useEffect(() => {
    ...
    const rotation = setInterval(pickRandom, 12000);
    setMounted(true);
  ...
  useEffect(() => {
    ...
    const additions: Array<{ id: string; kind: 'aviso' | 'lembrete' | 'mensagem'; title: string; body: string; createdAt: string; seen: boolean }> = [];

    const goalId = `goal-${key}`;
    if ((userProgress?.totalMinutes || 0) < (dailyGoal || 0) && !inboxItems.some((i) => i.id === goalId)) {
      additions.push({ id: goalId, kind: 'lembrete', title: 'Meta diária pendente', body: `Você está com ${userProgress?.totalMinutes || 0}/${dailyGoal || 0} min hoje.`, createdAt: new Date().toISOString(), seen: false });
    }
    ...
    if (iePlan && iePlan.currentDay <= 14) {
        ...
        if (!isDoneToday) {
          ...
          if (!inboxItems.some((i) => i.id === iePlanId)) {
            additions.push({ id: iePlanId, kind: 'lembrete', title: '🧠 Plano de Inteligência Emocional', body: `Sua tarefa do Dia ${iePlan.currentDay} está pendente. Tire 5 minutinhos para focar em você hoje.`, createdAt: new Date().toISOString(), seen: false });
          }
        }
      }

      if (todayMood?.primaryEmotion) {
      const moodId = `mood-${key}`;
      if (!inboxItems.some((i) => i.id === moodId)) {
        additions.push({ id: moodId, kind: 'aviso', title: 'Seu humor de hoje', body: `${todayMood.primaryEmotion} • intensidade ${todayMood.intensity}/5.`, createdAt: new Date().toISOString(), seen: false });
      }
    }

    if (additions.length) setInboxItems([...additions, ...inboxItems]);
  }, [dailyGoal, userProgress?.totalMinutes, todayMood?.primaryEmotion, todayMood?.intensity, iePlan]);
    ...
    const pullBroadcasts = async () => {
      try {
        ...
        const incoming = data.items
          ...
          .filter((item: any) => !inboxItems.some((i) => i.id === item.id));

        if (incoming.length) setInboxItems([...incoming, ...inboxItems]);
      } catch {}
    };
  ...
  const isPro = (window as any).__SERENO_IS_PRO__ || false; // Simple check for visual
  ...
  const features = [
    ...
    { icon: '🌬️', title: 'Respiração', desc: 'Técnicas relaxantes', tab: 'breathing' as Tab, color: 'bg-blue-100' },
    { icon: '🧘', title: 'Meditação', desc: 'Momentos de paz', tab: 'meditation' as Tab, color: 'bg-green-100' },
    { icon: '⏱️', title: 'Time Livre', desc: 'Meditação sem narração', tab: 'timer' as Tab, color: 'bg-sky-100' },
    { icon: '🌙', title: 'Yoga Nidra', desc: 'Relaxamento profundo', tab: 'yoga' as Tab, color: 'bg-indigo-100', isPro: true },
    { icon: '✨', title: 'Ho\'oponopono', desc: 'Práticas de liberação', tab: 'hooponopono' as Tab, color: 'bg-teal-100', isPro: true },
    { icon: '💖', title: 'Linguagens do Amor', desc: 'Descubra sua forma de amar', tab: 'lovelanguages' as Tab, color: 'bg-rose-100', isPro: true },
    ...
    { icon: '✋', title: 'Método dos 5 Dedos', desc: 'Comunicação e consciência', tab: 'fivefingers' as Tab, color: 'bg-indigo-50', isPro: true },
    { icon: '🌻', title: 'Meus Hábitos', desc: 'Sua jornada diária', tab: 'habits' as Tab, color: 'bg-emerald-100' },
    { icon: '🧭', title: 'Exploração Vocacional', desc: 'Descubra sua carreira ideal', tab: 'vocacional' as Tab, color: 'bg-indigo-100', isPro: true },
    { icon: '🧪', title: 'Inteligência Emocional', desc: 'Teste Goleman + Gardner', tab: 'emocional' as Tab, color: 'bg-violet-100' },
    ...
    { icon: '🎯', title: 'Plano de Segurança', desc: 'Contatos e estratégias', tab: 'safety' as Tab, color: 'bg-red-100' },
    { icon: '🎧', title: 'Mixer Sonoro', desc: 'Crie seu ambiente sonoro', tab: 'mixer' as Tab, color: 'bg-indigo-200' },
    { icon: '🏆', title: 'Conquistas (Badges)', desc: 'Veja seu progresso', tab: 'badges' as Tab, color: 'bg-emerald-100' },
    ...
    { icon: '🎯', title: 'Missões & Desafios', desc: 'Desafios de 7/21/30 dias', tab: 'missions' as Tab, color: 'bg-lime-100' },
    ...
    { icon: '🛤️', title: 'Trilhas Temáticas', desc: 'Luto, burnout, autoestima...', tab: 'tracks' as Tab, color: 'bg-indigo-50' },
    { icon: '📚', title: 'Psicoeducação', desc: 'Cards rápidos de 1 minuto', tab: 'psychoedu' as Tab, color: 'bg-blue-50' },
    ...
    { icon: '💑', title: 'Modo Casal', desc: 'Check-in compartilhado', tab: 'couple' as Tab, color: 'bg-pink-50' },
    { icon: '👨‍👩‍👧', title: 'Modo Família', desc: 'Acompanhar filhos/adolescentes', tab: 'family' as Tab, color: 'bg-amber-50' },
    { icon: '🏆', title: 'Compartilhar Conquista', desc: 'Stories e redes sociais', tab: 'share' as Tab, color: 'bg-lime-50' },
  ...
  const panelOptions = [
    ...
    { id: 'streak', title: 'Sequência (dias)', tab: 'habits' as Tab, icon: '🔥', value: userProgress.streak || 0 },
  ].filter((v, i, arr) => arr.findIndex((x) => x.id === v.id) === i);

  const selectedPanel = homePanelTabs
    .map((id) => panelOptions.find((p) => p.id === id))
    .filter(Boolean) as typeof panelOptions;

  const categoryGroups = [
    {
      name: 'Crise e Regulação Imediata',
      icon: '🆘',
      subtitle: 'Acalmar rápido e retomar controle',
      tone: 'crise',
      items: ['SOS Ansiedade','Plano de Segurança','Respiração','Meditação','Time Livre','Mixer Sonoro','Yoga Nidra','Ho\'oponopono','Micro-tarefas'],
    },
    {
      name: 'Autoconhecimento e Terapia Pessoal',
      icon: '🧠',
      subtitle: 'Entender emoções e curar padrões',
      tone: 'terapia',
      items: ['Diário (RPD)','Diário de Humor','Solta Aqui','Carta Terapêutica','Gratidão','Psicoeducação','Falas Tóxicas','Dicionário Emocional','Perfil de Regulação','Mapa Mental Emocional','Eu Mais Saudável','Cápsula do Tempo'],
    },
    {
      ...
      tone: 'progresso',
      items: ['Trilhas Temáticas','Missões & Desafios','Meus Hábitos','Estatísticas','Conquistas (Badges)','Calendário de Terapia'],
    },
    {
    ...
    },
    {
      ...
      tone: 'comunidade',
      items: ['Chat de Apoio','Mural de Esperança','Compartilhar Conquista','Indicar para Amigo','Sugestões'],
    },
    {
      ...
      tone: 'explorar',
      items: ['Abordagens psicológicas','Exploração Vocacional','Inteligência Emocional','Modo Arte'],
    },
  ];

  const bgPreview = panelEditorOpen ? conquestsBgDraft : conquestsBg;
  ...
  const conquestsStyle: any =
    bgPreview.mode === 'color'
      ? { backgroundImage: `linear-gradient(rgba(10,10,20,${overlayAlpha}), rgba(10,10,20,${overlayAlpha})), linear-gradient(135deg, ${bgPreview.color1}, ${bgPreview.color2})` }
      : bgPreview.mode === 'image' && bgPreview.image
        ? {
            backgroundImage: `linear-gradient(rgba(10,10,20,${overlayAlpha}), rgba(10,10,20,${overlayAlpha})), url(${bgPreview.image})`,
            backgroundSize: 'cover',
            backgroundPosition: 'center',
          }
        : { backgroundImage: `linear-gradient(rgba(10,10,20,${overlayAlpha}), rgba(10,10,20,${overlayAlpha})), ${presetGradient()}` };

  const toneClass = (tone: string) => {
    if (darkMode) {
      if (tone === 'crise') return 'from-rose-900/25 to-red-900/10 border-rose-800/60';
      if (tone === 'terapia') return 'from-indigo-900/25 to-violet-900/10 border-indigo-800/60';
      if (tone === 'progresso') return 'from-emerald-900/25 to-teal-900/10 border-emerald-800/60';
      if (tone === 'relacoes') return 'from-pink-900/25 to-rose-900/10 border-pink-800/60';
      if (tone === 'comunidade') return 'from-cyan-900/25 to-blue-900/10 border-cyan-800/60';
      return 'from-amber-900/25 to-orange-900/10 border-amber-800/60';
    }
    if (tone === 'crise') return 'from-rose-50 to-red-50 border-rose-100';
    if (tone === 'terapia') return 'from-indigo-50 to-violet-50 border-indigo-100';
    if (tone === 'progresso') return 'from-emerald-50 to-teal-50 border-emerald-100';
    if (tone === 'relacoes') return 'from-pink-50 to-rose-50 border-pink-100';
    if (tone === 'comunidade') return 'from-cyan-50 to-blue-50 border-cyan-100';
    return 'from-amber-50 to-orange-50 border-amber-100';
  };

  const favoriteFeatures = features.filter((f) => favoriteTabs.includes(String(f.tab)));
  const toggleFavorite = (tab: Tab) => {
    const id = String(tab);
    setFavoriteTabs((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [id, ...prev].slice(0, 8));
  };
    ...
    <div className="p-4 space-y-6 animate-fade-in pb-32">
      {showBirthdayCelebration && (
        <div className="fixed inset-0 z-[120] bg-black/65 backdrop-blur-sm flex items-start justify-center p-4 pt-16" onClick={() => setShowBirthdayCelebration(false)}>
          <div className={`relative w-full max-w-xl rounded-[2rem] p-8 text-center border overflow-hidden ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`} onClick={(e) => e.stopPropagation()}>
            <div className="absolute inset-0 pointer-events-none overflow-hidden">
              {Array.from({ length: 28 }).map((_, i) => (
                <span
                  key={i}
      ...
      <div className="text-center pt-2 animate-fade-in relative">
        <div className="absolute -top-6 left-0">
          <button
            onClick={onShowPlans}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-[10px] font-black uppercase tracking-wider transition-all active:scale-95 shadow-lg ${darkMode ? 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30' : 'bg-indigo-600 text-white border border-indigo-600'}`}
          >
            <DiamondIcon className="w-3 h-3" />
            Planos
          </button>
        </div>
        <h1 className={`text-2xl font-extrabold mb-1 ${darkMode ? 'text-slate-100' : 'text-slate-900'}`}>{greetingText}{userName ? `, ${userName}` : ''}</h1>
      </div>
      ...
      {isBirthdayToday && (
        <button
          ...
          className={`w-full relative overflow-hidden rounded-2xl border px-4 py-4 ${darkMode ? 'bg-gradient-to-r from-[#101a38] via-[#1a2450] to-[#101a38] border-indigo-700/60 text-white' : 'bg-gradient-to-r from-amber-50 via-yellow-50 to-amber-50 border-amber-200 text-amber-900'} shadow-[0_0_30px_rgba(250,204,21,0.2)]`}
        >
          <div className="absolute inset-0 opacity-30 bg-[radial-gradient(circle_at_20%_20%,#facc15,transparent_35%),radial-gradient(circle_at_80%_80%,#fb7185,transparent_35%)]" />
          <div className="relative text-center">
            <div className="text-3xl mb-1">🎂</div>
            <p className="text-xl font-black tracking-wide">FELIZ ANIVERSÁRIO!</p>
            <p className="text-sm font-bold opacity-95">Hoje o dia é todo seu! Que seja leve e especial. ✨</p>
          </div>
        </button>
      )}

      {/* Suas Conquistas */}
      <section className={`rounded-[2rem] border p-4 sm:p-5 ${darkMode ? 'border-indigo-900/60' : 'border-indigo-200'}`} style={conquestsStyle}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl border ${darkMode ? 'bg-white/10 border-white/20' : 'bg-white border-indigo-100'}`}>🌟</div>
            <div>
              <h3 className={`text-2xl font-black leading-none whitespace-nowrap ${darkMode ? 'text-white' : 'text-slate-900'}`}>Suas Conquistas</h3>
              <p className={`text-sm font-semibold whitespace-nowrap ${darkMode ? 'text-white/80' : 'text-slate-600'}`}>Continue cuidando de você!</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setInboxOpen(true)} className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-lg flex items-center gap-1 ${darkMode ? 'bg-white/15 text-white' : 'bg-indigo-600 text-white'}`}>
              🔔 Avisos {inboxItems.length > 0 ? `(${inboxItems.length})` : ''}
            </button>
            <button onClick={() => setPanelEditorOpen(true)} className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-lg ${darkMode ? 'bg-white/15 text-white' : 'bg-indigo-600 text-white'}`}>Personalizar painel</button>
          </div>
        </div>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-wider">
            ...
            <span className={darkMode ? 'text-white' : 'text-slate-800'}>{userProgress?.totalMinutes || 0}/{dailyGoal || 0} min</span>
          </div>
          <div className={`mt-1 h-2 rounded-full overflow-hidden ${darkMode ? 'bg-white/15' : 'bg-indigo-100'}`}>
            <div
              className="h-full rounded-full bg-gradient-to-r from-indigo-400 to-violet-400"
              style={{ width: `${Math.max(0, Math.min(100, dailyGoal > 0 ? ((userProgress?.totalMinutes || 0) / dailyGoal) * 100 : 0))}%` }}
            />
          </div>
          <button onClick={() => onNavigate('timer')} className={`mt-3 text-[11px] font-black uppercase px-3 py-1.5 rounded-xl ${darkMode ? 'bg-white/15 text-white' : 'bg-indigo-600 text-white'}`}>Começar agora</button>
        </div>

        {false && (
          <div className={`mt-4 p-3 rounded-2xl border ${darkMode ? 'bg-black/20 border-white/15' : 'bg-white border-slate-200'}`}>
            <p className={`text-[11px] mb-2 ${darkMode ? 'text-white/70' : 'text-slate-500'}`}>Escolha os botões do painel inicial:</p>
            <div className="grid grid-cols-2 gap-2 max-h-44 overflow-y-auto pr-1">
              {panelOptions.map((opt) => {
                const selected = homePanelTabs.includes(opt.id);
                return (
                  <button
                    key={opt.id}
                    onClick={() => setHomePanelTabs((prev) => selected ? prev.filter((id) => id !== opt.id) : [...prev, opt.id].slice(0, 12))}
                    className={`text-left p-2 rounded-xl border text-xs ${selected ? 'border-indigo-400 bg-indigo-500/20 text-white' : (darkMode ? 'border-white/20 bg-white/5 text-white/85' : 'border-slate-200 bg-slate-50 text-slate-700')}`}
                  >
                    <span className="font-bold">{opt.icon} {opt.title}</span>
                  </button>
                );
              })}
            </div>
        ...
        <div className="mt-4 grid grid-cols-4 gap-2">
          {selectedPanel.slice(0, 8).map((item) => (
            <button
              key={item.id}
              onClick={() => onNavigate(item.tab)}
              className={`min-h-[118px] rounded-2xl border p-2 text-center transition-all active:scale-95 ${darkMode ? 'bg-white/8 border-white/15 text-white' : 'bg-white border-slate-200 text-slate-700'}`}
            >
              <p className="text-xl leading-none">{item.icon}</p>
              <p className={`mt-3 text-4xl font-black leading-none ${darkMode ? 'text-white' : 'text-slate-900'}`}>{String(item.value)}</p>
              <p className={`mt-2 text-[10px] font-black uppercase leading-tight ${darkMode ? 'text-white/85' : 'text-slate-600'}`}>{item.title}</p>
            </button>
          ))}
        </div>
      </section>

      <div className={`relative overflow-hidden rounded-2xl px-5 py-4 border ${darkMode ? 'bg-gradient-to-br from-fuchsia-900/30 via-indigo-900/30 to-cyan-900/20 border-fuchsia-700/50 text-slate-100' : 'bg-gradient-to-br from-fuchsia-50 via-indigo-50 to-cyan-50 border-fuchsia-200 text-slate-700'} shadow-[0_10px_30px_rgba(99,102,241,0.2)]`}>
        <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_top_right,#a855f7,transparent_40%),radial-gradient(circle_at_bottom_left,#06b6d4,transparent_35%)]" />
        <p className="relative text-base font-extrabold leading-relaxed">{quote?.text}</p>
      </div>

      {favoriteFeatures.length > 0 && (
        <section className={`rounded-2xl border p-3 ${darkMode ? 'bg-slate-900/70 border-slate-700' : 'bg-white border-slate-200'}`}>
          <p className={`text-xs font-black uppercase mb-2 ${darkMode ? 'text-amber-300' : 'text-amber-700'}`}>⭐ Favoritos</p>
          <div className="grid grid-cols-2 gap-2">
            {favoriteFeatures.map((f) => (
              <button key={String(f.tab)} onClick={() => onNavigate(f.tab)} className={`p-2 rounded-xl text-left border ${darkMode ? 'bg-slate-800 border-slate-700 text-slate-100' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                <p className="text-sm font-black">{f.icon} {f.title}</p>
              </button>
            ))}
          </div>
          <div className={`mt-3 pt-3 border-t flex gap-2 ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
            <button onClick={() => setForceAllCategoriesOpen(true)} className={`flex-1 py-2 rounded-xl text-xs font-black ${darkMode ? 'bg-slate-800 text-slate-200 border border-slate-700' : 'bg-white border border-slate-200 text-slate-700'}`}>Expandir cards</button>
            <button onClick={() => setForceAllCategoriesOpen(false)} className={`flex-1 py-2 rounded-xl text-xs font-black ${darkMode ? 'bg-slate-800 text-slate-200 border border-slate-700' : 'bg-white border border-slate-200 text-slate-700'}`}>Recolher cards</button>
          </div>
        </section>
      )}

      {panelEditorOpen && (
        <div className="fixed inset-0 z-[130] bg-black/55 p-4" onClick={() => { setConquestsBg(conquestsBgDraft); setPanelEditorOpen(false); }}>
          <div className={`max-w-md mx-auto mt-10 rounded-2xl p-4 border shadow-xl max-h-[85vh] overflow-y-auto ${darkMode ? 'bg-[#0f1b32] border-[#30466d] text-white' : 'bg-white border-slate-200 text-slate-800'}`} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-black text-sm">Personalizar painel</h4>
            </div>

            <div className={`mb-3 rounded-xl p-3 border ${darkMode ? 'border-white/20 bg-white/5' : 'border-slate-200 bg-slate-50'}`}>
              <p className="text-[11px] font-black mb-2">Pré-visualização</p>
              <div className="rounded-xl p-3 border border-white/20" style={conquestsStyle}>
                <p className="text-sm font-black text-white">Suas Conquistas</p>
                <p className="text-xs text-white/90">Continue cuidando de você!</p>
              </div>
            </div>

            <p className={`text-[11px] mb-2 ${darkMode ? 'text-white/70' : 'text-slate-500'}`}>Escolha os botões do painel inicial:</p>
            <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1 mb-4">
              {panelOptions.map((opt) => {
                const selected = homePanelTabs.includes(opt.id);
                return (
                  <button
            ...
            <h5 className="text-[11px] font-black mb-2">Fundo do card “Suas Conquistas”</h5>

            <div className="grid grid-cols-3 gap-2 mb-3">
              {[
                ['default','Padrão'],['aurora','Aurora'],['calm','Calm'],['sunset','Sunset'],['forest','Forest'],['lavender','Lavender'],['ocean','Ocean'],['rosegold','Rosegold'],['midnight','Midnight']
              ].map(([id,label]) => (
                <button
                  key={id}
                  onClick={() => setConquestsBgDraft((p) => ({ ...p, mode: 'preset', preset: id as any }))}
                  className={`py-2 rounded-xl text-[11px] font-black border ${conquestsBgDraft.mode==='preset' && conquestsBgDraft.preset===id ? 'bg-indigo-600 text-white border-indigo-500' : (darkMode ? 'bg-white/10 border-white/20 text-white/90' : 'bg-slate-100 border-slate-200 text-slate-700')}`}
                >{label}</button>
              ))}
            </div>
            ...
            <div className="space-y-2 mb-3">
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    key={idx}
                    onClick={() => setConquestsBgDraft((p) => ({ ...p, mode: 'color', color1: c1, color2: c2 }))}
                    className="h-10 rounded-xl border border-white/20"
                    style={{ background: `linear-gradient(135deg, ${c1}, ${c2})` }}
                  />
                ))}
              </div>
              <div className="flex gap-2">
                <input type="color" value={conquestsBgDraft.color1} onChange={(e) => setConquestsBgDraft((p) => ({ ...p, mode: 'color', color1: e.target.value }))} className="w-full h-10 rounded-lg" />
                <input type="color" value={conquestsBgDraft.color2} onChange={(e) => setConquestsBgDraft((p) => ({ ...p, mode: 'color', color2: e.target.value }))} className="w-full h-10 rounded-lg" />
              </div>
            </div>

            <div className={`p-2 rounded-xl border ${darkMode ? 'border-white/20 bg-white/5' : 'border-slate-200 bg-slate-50'}`}>
              <div className="flex items-center justify-between mb-2">
                <p className="text-[11px] font-black">🖼️ Imagem de fundo (upload)</p>
                <button onClick={() => setConquestsBgDraft((p) => ({ ...p, mode: 'preset', preset: 'default', image: '' }))} className={`px-2 py-1 rounded-lg text-[11px] font-bold ${darkMode ? 'bg-white/10' : 'bg-white border border-slate-200'}`}>Remover imagem</button>
              </div>
              <label className={`w-full flex items-center justify-center gap-2 py-2 rounded-xl border text-[12px] font-bold cursor-pointer ${darkMode ? 'border-white/20 bg-white/10 text-white' : 'border-slate-200 bg-white text-slate-700'}`}>
                ⬆️ Enviar imagem
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => {
                    ...
                    reader.onload = () => {
                      ...
                      if (!result) return;
                      setConquestsBgDraft((p) => ({ ...p, mode: 'image', image: result }));
                    };
      ...
      {inboxOpen && (
        <div className="fixed inset-0 z-[120] bg-black/50 p-4" onClick={() => setInboxOpen(false)}>
          <div className={`max-w-md mx-auto mt-16 rounded-2xl p-4 border max-h-[70vh] overflow-y-auto ${darkMode ? 'bg-slate-900 border-slate-700 text-slate-100' : 'bg-white border-slate-200 text-slate-800'}`} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-black">🔔 Avisos</h4>
            </div>
            <div className="flex items-center gap-2 mb-3">
              <button className={`text-[11px] px-2 py-1 rounded-lg font-bold ${darkMode ? 'bg-slate-800' : 'bg-slate-100'}`} onClick={() => setInboxItems((prev) => prev.map((i) => ({ ...i, seen: true })))}>Marcar tudo como visto</button>
              <button className={`text-[11px] px-2 py-1 rounded-lg font-bold ${darkMode ? 'bg-red-900/40 text-red-200' : 'bg-red-100 text-red-700'}`} onClick={() => setInboxItems([])}>Excluir tudo</button>
            </div>
            <div className="space-y-2">
              {inboxItems.length === 0 && <p className="text-sm opacity-70">Sem avisos por enquanto.</p>}
              {inboxItems.map((item) => (
                <div
                  key={item.id}
                  className={`w-full text-left p-3 rounded-xl border ${item.seen ? (darkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200') : (darkMode ? 'bg-indigo-900/30 border-indigo-700' : 'bg-indigo-50 border-indigo-200')}`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <button
                      onClick={() => {
                        setSelectedInboxId(item.id);
                        setInboxItems((prev) => prev.map((i) => i.id === item.id ? { ...i, seen: true } : i));
                      }}
                      className="text-left flex-1"
                    >
                      <p className="text-xs font-black uppercase">{item.kind}</p>
                      <p className="font-bold text-sm mt-0.5">{item.title}</p>
                      <p className="text-xs opacity-80 mt-1">{item.id === selectedInboxId ? item.body : `${item.body}`}</p>
                    </button>
                    <button onClick={() => setInboxItems((prev) => prev.filter((i) => i.id !== item.id))} className={`text-xs px-2 py-1 rounded ${darkMode ? 'bg-slate-700 text-slate-200' : 'bg-white border border-slate-200 text-slate-600'}`}>Excluir</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="space-y-3">
        {categoryGroups.map((group) => {
          ...
          const open = forceAllCategoriesOpen !== null ? forceAllCategoriesOpen : !!openCategories[key];
          const groupFeatures = features.filter((f) => group.items.includes(f.title));
          return (
            <div key={group.name} className={`rounded-2xl border overflow-hidden ${darkMode ? 'text-slate-100 border-[#4c2d57] bg-gradient-to-br from-[#3a1130] via-[#22163d] to-[#141f3f]' : `bg-gradient-to-br ${toneClass(group.tone)} text-slate-800`}`}>
              <button
                onClick={() => setOpenCategories((prev) => ({ ...prev, [key]: !open }))}
                className="w-full p-3 flex items-center justify-between text-left"
              >
                <div>
                  <p className="text-[22px] leading-none">{group.icon}</p>
                  <p className="text-lg font-black leading-tight mt-1">{group.name}</p>
                  <p className={`text-[12px] ${darkMode ? 'text-slate-300/80' : 'text-slate-600'}`}>{group.subtitle}</p>
                </div>
              ...
              {open && (
                <div className="px-3 pb-3 grid grid-cols-2 gap-3">
                  {groupFeatures.map((f) => {
                    const fav = favoriteTabs.includes(String(f.tab));
                    return (
                      <div key={f.title} className={`rounded-[22px] text-left border overflow-hidden shadow-sm ${darkMode ? 'bg-gradient-to-br from-[#16233f] via-[#1b2a49] to-[#0f1a31] border-[#334a72]' : 'bg-white border-slate-200'}`}>
                        <div className="p-4 min-h-[142px] flex flex-col justify-between">
                          <div className="flex items-start justify-between gap-2">
                            <span className="text-3xl leading-none">{f.icon}</span>
                            <button onClick={() => toggleFavorite(f.tab)} className={`text-lg ${fav ? 'text-amber-400' : (darkMode ? 'text-slate-500' : 'text-slate-400')}`} title="Favoritar">{fav ? '★' : '☆'}</button>
                          </div>
                          <button onClick={() => onNavigate(f.tab)} className="text-left mt-3">
                            <p className={`text-[15px] font-black leading-tight tracking-[-0.01em] ${darkMode ? 'text-white' : 'text-slate-900'}`}>{f.title}</p>
                            <p className={`text-[13px] mt-1 leading-snug ${darkMode ? 'text-slate-300/85' : 'text-slate-500'}`}>{f.desc}</p>
                          </button>
                        </div>
                      </div>
                    );
  ...
  return (
    <div className={`p-4 animate-fade-in pb-32 ${dm ? 'text-slate-100' : 'text-gray-800'}`}>
      <div className="text-center mb-10 pt-8">
        <h2 className={`text-4xl font-extrabold tracking-tight mb-2 ${dm ? 'text-red-400' : 'bg-gradient-to-r from-red-600 to-orange-500 text-transparent bg-clip-text'}`}>SOS Ansiedade</h2>
        <p className={`mt-2 font-medium ${dm ? 'text-slate-400' : 'text-gray-500'}`}>Ferramentas rápidas para momentos de crise.</p>
      </div>

      <div className="space-y-5 max-w-lg mx-auto">
        <button
          onClick={() => onNavigate('breathing', { exerciseId: '333', from: 'sos' })}
          className={`w-full p-6 rounded-3xl flex items-center justify-between transition-all duration-300 active:scale-95 text-left border shadow-sm hover:shadow-md group relative overflow-hidden ${dm ? 'bg-slate-800/80 border-slate-700 backdrop-blur-md' : 'bg-white border-blue-50 hover:border-blue-100'}`}
        >
        ...
        <button
          ...
          className={`w-full p-6 rounded-3xl flex items-center justify-between transition-all duration-300 active:scale-95 text-left border shadow-sm hover:shadow-md group relative overflow-hidden ${dm ? 'bg-slate-800/80 border-slate-700 backdrop-blur-md' : 'bg-white border-pink-50 hover:border-pink-100'}`}
        >
        ...
        <button
          ...
          className={`w-full p-6 rounded-3xl flex items-center justify-between transition-all duration-300 active:scale-95 text-left border shadow-sm hover:shadow-md group relative overflow-hidden ${dm ? 'bg-slate-800/80 border-slate-700 backdrop-blur-md' : 'bg-white border-teal-50 hover:border-teal-100'}`}
        >
...
function CalmSection({ onNavigateBack, darkMode: dm, initialStep, onStepChange }: { onNavigateBack?: () => void, darkMode?: boolean, initialStep?: number, onStepChange?: (step: number) => void }) {
...
function AcalmeseSection({ onNavigateBack, darkMode: dm, initialStep, onStepChange }: { onNavigateBack?: () => void, darkMode?: boolean, initialStep?: number, onStepChange?: (step: number) => void }) {
  const [currentStep, setCurrentStep] = useState(initialStep ?? -1);
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 border-t border-gray-50 animate-fade-in">
                    <div className="mt-3 space-y-3 text-sm">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); deleteRecord(record.id); }} className="text-xs text-red-500 hover:text-red-700 mt-2">🗑️ Excluir</button>
                    </div>
                  </div>
                )}
              </div>
...
function SafetyPlanSection({ safetyPlan, setSafetyPlan, darkMode: dm }: { safetyPlan: SafetyPlan; setSafetyPlan: (value: SafetyPlan) => void; darkMode?: boolean }) {
  ...
  const sections = [
    { key: 'warningSignals' as const, title: '⚠️ Sinais de Alerta', desc: 'Pensamentos, sentimentos ou situações que sinalizam uma crise', color: dm ? 'bg-amber-900/20 border-amber-800/50' : 'bg-yellow-50 border-yellow-200', titleColor: dm ? 'text-amber-300' : 'text-gray-800', btnColor: 'text-amber-500' },
    { key: 'copingStrategies' as const, title: '🛡️ Estratégias de Enfrentamento', desc: 'Coisas que posso fazer sozinho(a) para me acalmar', color: dm ? 'bg-blue-900/20 border-blue-800/50' : 'bg-blue-50 border-blue-200', titleColor: dm ? 'text-blue-300' : 'text-gray-800', btnColor: 'text-blue-500' },
    { key: 'reasonsToLive' as const, title: '💛 Motivos para Viver', desc: 'Pessoas, coisas e razões importantes para mim', color: dm ? 'bg-emerald-900/20 border-emerald-800/50' : 'bg-green-50 border-green-200', titleColor: dm ? 'text-emerald-300' : 'text-gray-800', btnColor: 'text-green-600' },
  ];
  ...
  return (
    <div className="p-4 animate-fade-in overflow-y-auto pb-24 max-w-lg mx-auto">
      <div className="text-center mb-8 pt-4">
        ...
        <h2 className={`text-3xl font-extrabold tracking-tight mb-2 ${dm ? 'text-slate-100' : 'text-gray-900'}`}>Plano de Segurança</h2>
        <p className={`font-medium ${dm ? 'text-slate-400' : 'text-gray-600'}`}>Seu plano pessoal para momentos difíceis</p>
      </div>
        ...
        <div key={section.key} className={`${section.color} rounded-3xl p-5 border mb-4 animate-slide-up shadow-sm`} style={{ animationDelay: `${idx * 0.05}s` }}>
            ...
            <div className="space-y-2 mb-4">
              {safetyPlan[section.key].map((item, i) => (
                <div key={i} className={`flex items-center justify-between gap-2 rounded-2xl px-4 py-3 shadow-sm ${dm ? 'bg-slate-800/80' : 'bg-white/80 backdrop-blur-sm'}`}>
                  ...
                  <button onClick={() => removeFromList(section.key, i)} className="text-red-400 hover:text-red-600 p-1 flex-shrink-0">
          ...
          {editingSection === section.key ? (
            <div className="flex gap-2 animate-fade-in">
              ...
              <button onClick={() => addToList(section.key)} className={`px-4 py-3 rounded-2xl text-sm font-bold shadow-sm transition-all active:scale-95 ${dm ? 'bg-blue-600 text-white hover:bg-blue-500' : 'bg-blue-500 text-white hover:bg-blue-600'}`}>+</button>
              <button onClick={() => { setEditingSection(null); setTempInput(''); }} className={`px-4 py-3 rounded-2xl text-sm font-bold shadow-sm transition-all active:scale-95 ${dm ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>✕</button>
            </div>
          ) : (
            <button onClick={() => setEditingSection(section.key)} className={`text-sm font-bold flex items-center gap-1 active:scale-95 transition-all ${section.btnColor}`}>
      ...
      {/* Emergency Contacts */}
      <div className={`rounded-3xl p-5 border mb-4 animate-slide-up shadow-sm ${dm ? 'bg-purple-900/20 border-purple-800/50' : 'bg-purple-50 border-purple-200'}`} style={{ animationDelay: '0.15s' }}>
        <h3 className={`font-bold mb-1 ${dm ? 'text-purple-300' : 'text-purple-800'}`}>📞 Contatos de Emergência</h3>
        <p className={`text-sm mb-4 ${dm ? 'text-slate-400' : 'text-gray-500'}`}>Pessoas que posso ligar em uma crise</p>

        {safetyPlan.emergencyContacts.length > 0 && (
          <div className="space-y-2 mb-4">
            {safetyPlan.emergencyContacts.map((c, i) => (
              <div key={i} className={`flex items-center gap-3 rounded-2xl px-4 py-3 shadow-sm ${dm ? 'bg-slate-800/80' : 'bg-white/80 backdrop-blur-sm'}`}>
                <div className="flex-1 min-w-0">
                ...
                <a href={`tel:${c.phone}`} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all active:scale-95 ${dm ? 'bg-purple-500/20 text-purple-400 hover:bg-purple-500/30' : 'bg-purple-100 text-purple-600 hover:bg-purple-200'}`}>📞</a>
                <button onClick={() => removeContact(i)} className="w-10 h-10 rounded-xl flex items-center justify-center text-red-400 hover:text-red-600 transition-all hover:bg-red-50 active:scale-95 flex-shrink-0">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
                </button>
              </div>
            ))}
        ...
        {editingSection === 'contacts' ? (
          <div className="space-y-3 animate-fade-in">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={addContact} className={`px-4 py-3 rounded-2xl text-sm font-bold shadow-sm transition-all active:scale-95 ${dm ? 'bg-purple-600 text-white hover:bg-purple-500' : 'bg-purple-500 text-white hover:bg-purple-600'}`}>+</button>
              <button onClick={() => { setEditingSection(null); setTempName(''); setTempPhone(''); }} className={`px-4 py-3 rounded-2xl text-sm font-bold shadow-sm transition-all active:scale-95 ${dm ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>✕</button>
            </div>
          </div>
        ) : (
      ...
      <div className={`rounded-3xl p-5 border mb-8 animate-slide-up shadow-sm ${dm ? 'bg-slate-800/80 border-slate-700' : 'bg-gradient-to-r from-teal-50 to-emerald-50 border-teal-100'}`} style={{ animationDelay: '0.25s' }}>
        ...
        <input type="text" value={safetyPlan.professionalContact} onChange={(e) => setSafetyPlan({ ...safetyPlan, professionalContact: e.target.value })} placeholder="Nome e telefone..." className={`w-full px-4 py-3 rounded-2xl text-sm focus:outline-none focus:ring-2 shadow-sm transition-all ${dm ? 'bg-slate-700 text-white border border-slate-600 focus:ring-teal-500' : 'bg-white border border-white focus:ring-teal-500'}`} />
      </div>
    </div>
  );
}
...
function RemindersSection({ settings, setSettings, darkMode: dm }: { settings: ReminderSettings; setSettings: (value: ReminderSettings) => void; darkMode?: boolean }) {
  ...
  const toggleReminder = (key: 'moodReminder' | 'breathingReminder' | 'diaryReminder' | 'healthySelfReminder' | 'meditationReminder' | 'gratitudeReminder' | 'sleepReminder' | 'missionsReminder' | 'microtasksReminder') => {
    if (permissionStatus !== 'granted') {
      requestPermission();
      return;
    }
    if (key === 'healthySelfReminder' && healthyMessages.length === 0) {
      alert('Salve pelo menos 1 mensagem em “Eu Mais Saudável” para ativar este lembrete.');
      return;
    }
    setSettings({ ...settings, [key]: !settings[key] });
  };

  const updateTime = (key: 'moodTime' | 'breathingTime' | 'diaryTime' | 'healthySelfTime' | 'meditationTime' | 'gratitudeTime' | 'sleepTime' | 'missionsTime' | 'microtasksTime', value: string) => {
  ...
  useEffect(() => {
    const checkReminders = () => {
      ...
      if (settings.microtasksReminder && currentTime === settings.microtasksTime) {
        setLastNotifyKey(minuteKey);
        new Notification('SerenAI - Microtarefas', { body: '🌿 Faça uma microtarefa de 5 minutos para sair da paralisia.', icon: '🧠' });
        return;
      }
...
function YogaNidraSection({ onComplete, darkMode: dm, onCheckAccess, defaultVoice, setDefaultVoice }: { onComplete?: () => void; darkMode?: boolean; onCheckAccess: (feature: string, action?: string) => boolean; defaultVoice: 'masculino' | 'feminino' | 'nenhuma'; setDefaultVoice: (v: 'masculino' | 'feminino' | 'nenhuma') => void }) {
  ...
  const [selectedEnv, setSelectedEnv] = useState<typeof yogaEnvironments[0] | null>(null);

  // Narration system states (same as MeditationSection)
  const [malePiperVoice, setMalePiperVoice] = useLocalStorage<string>('yoga_piper_voice_male', 'pt_BR-cadu-medium');
  const [femalePiperVoice, setFemalePiperVoice] = useLocalStorage<string>('yoga_piper_voice_female', 'pt-BR-FranciscaNeural');
  const [deviceSpeaking, setDeviceSpeaking] = useState(false);
  const [piperLoading, setPiperLoading] = useState(false);
  const [currentCueIndex, setCurrentCueIndex] = useState(0);
  const [elapsedMs, setElapsedMs] = useState(0);
  const [speedMode, setSpeedMode] = useLocalStorage<'0.9' | '1.0' | '1.1'>('yoga_speed_mode', '1.0');
  const [pauseMode, setPauseMode] = useLocalStorage<'curta' | 'normal' | 'profunda'>('yoga_pause_mode', 'profunda');
  const [captionOnly, setCaptionOnly] = useLocalStorage<boolean>('yoga_caption_only', false);
  const [sessionDone, setSessionDone] = useState(false);
  const [sessionStartedAt, setSessionStartedAt] = useState<number | null>(null);

  // Background sound states
  const [bgEnabled, setBgEnabled] = useLocalStorage<boolean>('yoga_bg_enabled', true);
  const [bgSoundId, setBgSoundId] = useLocalStorage<string>('yoga_bg_sound', 'meditacao');
  const [bgVolume, setBgVolume] = useLocalStorage<number>('yoga_bg_volume', 26);
  const [bgMixerEnabled, setBgMixerEnabled] = useLocalStorage<boolean>('yoga_bg_mixer', true);
  const [bgMixerTracks, setBgMixerTracks] = useLocalStorage<Record<string, number>>('yoga_bg_mixer_tracks', {});
  ...
  // Build narrative cues with pauses scaled to 20 min
  const narrativeCues = useMemo(() => {
    const raw = yogaNidraFullText.replace(/\s+/g, ' ').trim();
    if (!raw) return [] as Array<{ idx: number; text: string; startMs: number; endMs: number; pauseMs: number }>;

    const sentences = (raw.match(/[^.!?]+[.!?]?/g) || []).map(s => s.trim()).filter(Boolean);

    const base = sentences.map((s, idx) => {
      const words = s.split(/\s+/).length;
      const speechMs = Math.max(1000, words * 320);
      let pauseMs = /[!?]$/.test(s) ? 1700 : 1300;
      // Yoga Nidra has longer pauses for body scan and breathing
    ...
    const modeScale = pauseMode === 'curta' ? 0.75 : pauseMode === 'profunda' ? 1.45 : 1;
    const pauseScale = totalPause > 0
      ? Math.max(0.55, Math.min(6, ((targetMs - totalSpeech) / totalPause) * modeScale))
      : modeScale;

    let cursor = 0;
    return base.map((b) => {
      const pauseMs = Math.max(500, Math.round(b.pauseMs * pauseScale));
      const startMs = cursor;
      const endMs = cursor + b.speechMs;
      cursor = endMs + pauseMs;
      return { idx: b.idx, text: b.text, startMs, endMs, pauseMs };
    });
  }, [pauseMode]);
  ...
  const startNarrationFromCue = async (cueIdx: number) => {
    if (!narrativeCues.length) return;
    const safeIdx = Math.max(0, Math.min(cueIdx, narrativeCues.length - 1));
    stopPiperAudio();
    ...
    let elapsed = narrativeCues[safeIdx].startMs;
    setElapsedMs(elapsed);

    for (let i = safeIdx; i < narrativeCues.length; i++) {
      if (narrationRunRef.current !== runId) return;
      const cue = narrativeCues[i];
  ...
  // Manage bg sounds
  useEffect(() => {
    if (!sessionRunning || !bgEnabled) { stopAllBgSounds(); return; }
  ...
  }, [sessionRunning, bgEnabled, bgSoundId, bgVolume, bgMixerEnabled, bgMixerTracks]);
  ...
  const getSuggestedMixForEnv = (env: typeof yogaEnvironments[0]): Record<string, number> => {
  ...
  const startSession = (env: typeof yogaEnvironments[0]) => {
    setSelectedEnv(env);
  ...
  const jumpBySeconds = (deltaSec: number) => {
    ...
    const idx = narrativeCues.findIndex((c) => target >= c.startMs && target < c.endMs + c.pauseMs);
    startNarrationFromCue(idx >= 0 ? idx : (deltaSec > 0 ? narrativeCues.length - 1 : 0));
  };
      ...
      <div className={`p-4 animate-fade-in max-w-lg mx-auto pb-32 ${dm ? 'text-white' : ''}`}>
        ...
        <div className="flex justify-center gap-4 mb-4">
          ...
          <span className={`px-3 py-1.5 rounded-xl text-xs font-bold ${dm ? 'bg-indigo-900/40 text-indigo-300' : 'bg-indigo-50 text-indigo-700'}`}>
            Frase {Math.min(narrativeCues.length, currentCueIndex + 1)} de {narrativeCues.length}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-4">
          {!deviceSpeaking && !captionOnly && selectedVoice !== 'nenhuma' && (
            <button onClick={() => startNarrationFromCue(currentCueIndex)} className="px-4 py-2.5 rounded-xl bg-indigo-600 text-white text-xs font-bold">
              {piperLoading ? '⏳ Carregando...' : '▶️ Iniciar Narração'}
            </button>
          )}
  ...
  return (
    <div className="p-4 animate-fade-in overflow-y-auto pb-32 max-w-lg mx-auto">
      <div className="text-center mb-8 pt-4">
        <span className="text-5xl block mb-4 filter drop-shadow-md">🌙</span>
        <h2 className={`text-3xl font-extrabold tracking-tight mb-2 ${dm ? 'text-slate-100' : 'text-gray-900'}`}>Yoga Nidra</h2>
        <p className={`font-medium ${dm ? 'text-slate-400' : 'text-gray-600'}`}>Sono Yógico — Relaxamento Profundo</p>
      </div>
      ...
      {/* Settings trigger + Bottom Sheet */}
      <div className={`rounded-3xl border mb-6 overflow-hidden shadow-sm ${dm ? 'bg-slate-800/80 border-slate-700' : 'bg-white border-gray-100'}`}>
        <button onClick={() => setSettingsOpen(true)} className="w-full flex items-center justify-between p-5 text-left">
          <div>
            <h3 className={`font-bold text-sm ${dm ? 'text-slate-200' : 'text-gray-800'}`}>⚙️ Configurações</h3>
            <p className={`text-xs mt-0.5 ${dm ? 'text-slate-400' : 'text-slate-500'}`}>Voz: {selectedVoice} • Música: {bgEnabled ? (bgMixerEnabled ? 'Mixer' : 'ON') : 'OFF'} • Pausa: {pauseMode}</p>
          </div>
          <span className={`text-xs font-bold ${dm ? 'text-slate-300' : 'text-slate-600'}`}>Abrir</span>
        </button>
      </div>
      ...
      {settingsOpen && (
        <div className="fixed inset-0 z-[99999] bg-black/45 backdrop-blur-sm animate-fade-in" onPointerDown={(e) => { if (e.target === e.currentTarget) setSettingsOpen(false); }}>
          <div className={`absolute left-1/2 top-[42%] w-[94%] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-3xl p-4 border max-h-[85vh] overflow-y-auto animate-slide-up shadow-2xl ${dm ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
      ...
      <div className="mb-8">
        ...
        <p className={`text-sm font-medium mb-4 ${dm ? 'text-slate-500' : 'text-gray-500'}`}>Selecione para iniciar a sessão imersiva.</p>
