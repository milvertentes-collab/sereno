...
export default function MoodSection({ moodHistory, setMoodHistory, darkMode: dm, initialStep, onStepChange }: MoodSectionProps) {
    ...
    if (todayEntry && step === 0) {
        const primaryDetails = getEmotionDetails(todayEntry.primaryEmotion);
        ...
        return (
            <div className={`p-4 animate-fade-in pb-24 max-w-lg mx-auto ${dm ? 'text-white' : ''}`}>
                <div className="text-center mb-8 pt-4">
                    ...
                    <h2 className={`text-3xl font-extrabold tracking-tight ${c('text-gray-900', 'text-slate-100')}`}>Humor de Hoje</h2>
                    <p className={`font-medium mt-1 ${c('text-gray-600', 'text-slate-400')}`}>Você já registrou seu humor hoje!</p>
                </div>
                ...
                {primaryDetails && (
                    <div className={`rounded-3xl p-6 mb-6 shadow-sm border border-black/5 ${primaryDetails.color} ${dm ? 'opacity-90' : ''}`}>
                        ...
                        <button
                            ...
                            className={`w-full mt-6 py-3.5 rounded-2xl text-sm font-bold bg-white/40 hover:bg-white/60 transition-all active:scale-95 shadow-sm ${primaryDetails.textColor}`}
                        >
                            ✏️ Editar Registro
                        </button>
                    </div>
                ...
                {moodHistory.length > 1 && (
                    <div className={`rounded-3xl p-6 border shadow-sm ${c('bg-white border-gray-100', 'bg-slate-800/80 border-slate-700')}`}>
                        <h3 className={`font-bold mb-4 ${c('text-gray-800', 'text-slate-200')}`}>Últimos registros</h3>
                                    ...
                                    <div key={entry.id || index} className={`flex items-center gap-4 p-4 rounded-2xl ${e.color} ${dm ? 'opacity-90' : 'bg-opacity-40'} shadow-sm`}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className={`text-xs font-medium ${c('text-gray-600', 'text-slate-600')}`}>
                                                {isToday ? 'Hoje' : d.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: '2-digit' })} • Nível {entry.intensity}
                                            </div>
                                        </div>
                                    </div>
                                )
            ...
            {(step === 1 || step === 0) && (
                <div className="space-y-6">
                    <div className="text-center pt-4 mb-2">
                        <h2 className={`text-3xl font-extrabold tracking-tight ${c('text-gray-900', 'text-slate-100')}`}>
                            {primary ? 'Mais alguma emoção?' : 'Como você está hoje?'}
                        </h2>
                        <p className={`text-sm mt-2 font-medium ${c('text-gray-600', 'text-slate-400')}`}>
                            {primary ? 'Opcional: escolha até 2 emoções secundárias' : 'Escolha sua emoção principal'}
                        </p>
                        {!primary && (
                    ...
                    {!primary && Object.entries(EMOTION_CATEGORIES).map(([key, cat]) => (
                        <div key={key} className="mb-8 animate-slide-up">
                            <h3 className={`font-bold mb-4 px-1 text-lg ${c('text-gray-800', 'text-slate-300')}`}>{cat.title}</h3>
                            <div className="grid grid-cols-2 gap-3">
                                {cat.items.map(e => (
                                    <button
                                        ...
                                        className={`flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 rounded-3xl text-left transition-all active:scale-95 shadow-sm hover:shadow-md ${e.color} ${dm ? 'opacity-90 hover:opacity-100' : ''}`}
                                    >
                                        <span className="text-3xl filter drop-shadow-sm">{e.icon}</span>
                                        <div>
                                            <div className={`font-bold text-sm ${e.textColor}`}>{e.label}</div>
                                            <div className={`text-xs font-medium opacity-80 ${e.textColor} line-clamp-2 sm:line-clamp-1 mt-0.5`}>{e.desc}</div>
                                        </div>
                                    </button>
                                ))}
                            </div>
                    ...
                    {primary && (
                        <div className="animate-slide-up">
                            <div className={`p-5 rounded-3xl mb-6 flex items-center justify-between shadow-sm border border-black/5 ${getEmotionDetails(primary)?.color} ${dm ? 'opacity-90' : ''}`}>
                                <div className="flex items-center gap-4">
                                    <span className="text-4xl filter drop-shadow-sm">{getEmotionDetails(primary)?.icon}</span>
                                    <div>
                                        <p className={`text-xs uppercase font-extrabold opacity-70 mb-0.5 ${getEmotionDetails(primary)?.textColor}`}>Principal</p>
                                        <p className={`font-bold text-lg ${getEmotionDetails(primary)?.textColor}`}>{getEmotionDetails(primary)?.label}</p>
                                    </div>
                                </div>
                                <button onClick={() => setPrimary('')} className={`text-sm font-bold underline px-2 py-1 ${getEmotionDetails(primary)?.textColor}`}>Trocar</button>
                            </div>
                                ...
                                {getAllEmotions().filter(e => e.id !== primary).map(e => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            disabled={isDisabled}
                                            className={`p-3 rounded-2xl text-center flex flex-col items-center gap-1.5 transition-all shadow-sm active:scale-95 border
                              ${isSelected ? `${e.color} border-transparent ${dm ? 'opacity-90' : ''}` : c('bg-white border-gray-100 hover:bg-gray-50', 'bg-slate-800/80 border-slate-700 hover:bg-slate-700')}
                ...
                <div className="space-y-6 pt-4 animate-slide-up">
                    <div className="text-center mb-10 pt-4">
                        ...
                        <div className="flex justify-center mt-10 mb-4">
                            <span className="text-7xl animate-pulse filter drop-shadow-lg">{getEmotionDetails(primary)?.icon}</span>
                        </div>
                    </div>
                    ...
                    <div className={`p-6 sm:p-8 rounded-[2.5rem] border shadow-sm ${c('bg-white border-gray-100', 'bg-slate-800/80 border-slate-700')}`}>
                        <div className="flex justify-between w-full mb-6 relative">
                            <div className={`absolute top-1/2 left-0 w-full h-1.5 -translate-y-1/2 rounded-full z-0 ${c('bg-gray-100', 'bg-slate-700')}`}></div>
                            {[1, 2, 3, 4, 5].map(val => (
                                <button
                                    key={val}
                                    ...
                                    className={`relative z-10 w-12 h-12 sm:w-14 sm:h-14 rounded-full font-extrabold text-lg sm:text-xl flex items-center justify-center transition-all active:scale-95 ${intensity === val
                                        ? `scale-110 shadow-lg ${getEmotionDetails(primary)?.color} ${getEmotionDetails(primary)?.textColor} border-4 border-white dark:border-slate-800`
                        ...
                        <div className={`text-center font-bold mt-8 text-lg ${intensity >= 4 ? getEmotionDetails(primary)?.textColor : c('text-gray-700', 'text-slate-300')}`}>
                            <span className="opacity-60 text-sm block mb-1">Intensidade {intensity}</span>
                            {intensity === 1 && 'Muito Leve'}
                            {intensity === 2 && 'Leve'}
                            {intensity === 3 && 'Moderado'}
                            {intensity === 4 && 'Forte'}
                            {intensity === 5 && 'Muito Forte'}
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3 mt-10">
                        ...
                        <button onClick={() => changeStep(3)} className="flex-1 py-4 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-bold shadow-lg shadow-blue-500/30 transition-all active:scale-95">
                            Continuar
                        </button>
                    </div>
                </div>
            ...
            {step === 3 && (
                <div className="space-y-6 pt-4 animate-slide-up">
                    <div className="text-center mb-8 pt-4">
                        ...
                        <p className={`text-sm font-medium mt-3 ${c('text-gray-500', 'text-slate-400')}`}>Opcional: Descreva o que aconteceu ou o que está passando pela sua mente.</p>
                    </div>
                    ...
                    <textarea
                        ...
                        className={`w-full p-5 rounded-3xl border min-h-[180px] resize-none focus:outline-none focus:ring-2 shadow-inner transition-all ${c('bg-gray-50 border-gray-200 text-gray-800 focus:bg-white focus:ring-blue-500/50', 'bg-slate-800 border-slate-700 text-white focus:ring-blue-500/50')}`}
                    />
            ...
            {step === 4 && (
                <div className="space-y-6 pt-4 animate-slide-up">
                    <div className="text-center mb-6 pt-4">
                        <h2 className={`text-3xl font-extrabold tracking-tight ${c('text-gray-900', 'text-slate-100')}`}>O que causou isso?</h2>
                        <p className={`text-sm font-medium mt-3 ${c('text-gray-500', 'text-slate-400')}`}>Identificar o gatilho ajuda a entender seus padrões.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {TRIGGER_CATEGORIES.map(cat => (
                            <button
                                ...
                                className={`p-4 rounded-2xl text-center flex flex-col items-center gap-2 transition-all shadow-sm active:scale-95 border
                                    ${trigger === cat.id
                                        ? `bg-blue-100 border-blue-400 ${dm ? 'bg-blue-900/40 border-blue-700' : ''}`
                    ...
                    <textarea
                        ...
                        placeholder="Mais detalhes sobre o que aconteceu... (opcional)"
                        className={`w-full p-5 rounded-3xl border min-h-[120px] resize-none focus:outline-none focus:ring-2 shadow-inner transition-all ${c('bg-gray-50 border-gray-200 text-gray-800 focus:bg-white focus:ring-blue-500/50', 'bg-slate-800 border-slate-700 text-white focus:ring-blue-500/50')}`}
                    />
            ...
            {step === 7 && (
                <div className="space-y-8 pt-4 animate-slide-up">
                    <div className="text-center mb-6 pt-4">
                        ...
                        <p className={`text-sm font-medium mt-3 ${c('text-gray-500', 'text-slate-400')}`}>Essas métricas ajudam a entender sua saúde física e social.</p>
                    </div>
                    ...
                    <div className="space-y-6">
                            ...
                            <div className="flex justify-between items-center gap-2">
                                {[1, 2, 3, 4, 5].map(val => (
                                    <button
                                        ...
                                        className={`w-10 h-10 rounded-full font-bold transition-all active:scale-90 ${sleepQuality === val ? 'bg-indigo-500 text-white shadow-md' : c('bg-gray-100 text-gray-500', 'bg-slate-700 text-slate-400')}`}
                                    >
                                        {val}
                                    </button>
                                ))}
                            ...
                            <div className="flex justify-between items-center gap-2">
                                {[1, 2, 3, 4, 5].map(val => (
                                    <button
                                        ...
                                        className={`w-10 h-10 rounded-full font-bold transition-all active:scale-90 ${energyLevel === val ? 'bg-orange-500 text-white shadow-md' : c('bg-gray-100 text-gray-500', 'bg-slate-700 text-slate-400')}`}
                                    >
                                        {val}
                                    </button>
                                ))}
                            ...
                            <div className="flex justify-between items-center gap-2">
                                {[1, 2, 3, 4, 5].map(val => (
                                    <button
                                        ...
                                        className={`w-10 h-10 rounded-full font-bold transition-all active:scale-90 ${sociability === val ? 'bg-green-500 text-white shadow-md' : c('bg-gray-100 text-gray-500', 'bg-slate-700 text-slate-400')}`}
                                    >
                                        {val}
                                    </button>
                                ))}
                        ...
                        <div className={`p-6 rounded-[2rem] border shadow-sm ${c('bg-white border-gray-100', 'bg-slate-800/80 border-slate-700')}`}>
                            ...
                            <input
                                ...
                                className={`w-full p-3 rounded-xl border text-sm ${c('bg-gray-50 border-gray-200', 'bg-slate-900 border-slate-700')}`}
                            />
                        </div>
                        ...
                        <div className={`p-6 rounded-[2rem] border shadow-sm ${c('bg-white border-gray-100', 'bg-slate-800/80 border-slate-700')}`}>
                            <h3 className="font-bold mb-3">🌙 Modo pré-menstrual (opcional)</h3>
                            <div className="grid grid-cols-3 gap-2">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`px-2 py-2 rounded-lg text-xs font-bold border ${menstrualMode === phase.id ? 'bg-purple-500 text-white border-purple-500' : c('bg-gray-50 border-gray-200 text-gray-700', 'bg-slate-900 border-slate-700 text-slate-300')}`}
                                    >
                                        {phase.label}
                                    </button>
                                ))}
