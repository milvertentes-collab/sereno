                ...
                <div className="max-w-3xl mx-auto space-y-6">
                    {savedLetters.length === 0 ? (
                        ...
                        savedLetters.map(letter => {
                            ...
                            return (
                                <div key={letter.id} className={`p-6 rounded-3xl border shadow-sm relative overflow-hidden ${dm ? 'bg-slate-800 border-slate-700' : 'bg-white border-amber-100'}`}>
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-3">
                                            <span className="text-2xl">{letter.status === 'ripped' ? '🗡️' : '📔'}</span>
                                            <div>
                                                <h4 className="font-extrabold text-lg">{letter.title}</h4>
                                                <p className="text-xs opacity-60 font-semibold">{new Date(letter.date).toLocaleString('pt-BR')}</p>
                                            </div>
                                        </div>
                                        {daysLeft !== null && (
                                    ...
                                    <div className={`p-4 rounded-xl text-sm leading-relaxed whitespace-pre-wrap font-serif italic ${dm ? 'bg-slate-900/50 text-slate-300' : 'bg-amber-50/50 text-slate-700'}`}>
                                        {letter.content}
                                    </div>
                                </div>
                            )
                    ...
                    <>
                        <div className="flex justify-center mb-8">
                            <button 
                                ...
                                className={`px-6 py-3 rounded-full font-bold shadow-md transition-all active:scale-95 flex items-center gap-2 ${dm ? 'bg-slate-800 text-amber-300 border border-slate-700 hover:bg-slate-700' : 'bg-white text-amber-800 border border-amber-200 hover:bg-amber-50'}`}
                            >
                                🗃️ Ver Minha Gaveta de Cartas
                            </button>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 animate-slide-up">
                            {letterModels.map(model => (
                                <button 
                                    ...
                                    className={`p-5 rounded-3xl border text-left transition-all active:scale-95 hover:shadow-md group flex flex-col items-center text-center gap-3 ${dm ? 'bg-slate-800 border-slate-700 hover:border-amber-500/50' : `bg-gradient-to-br ${model.bg} bg-opacity-30 border-transparent hover:border-white/50 shadow-sm`}`}
                                >
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl shadow-sm ${dm ? 'bg-slate-700/80' : 'bg-white/60'}`}>
                                        {model.icon}
                                    </div>
                                    <div>
                                        <h3 className={`font-extrabold text-sm mb-1 ${dm ? 'text-slate-200 group-hover:text-amber-400' : model.text}`}>{model.title}</h3>
